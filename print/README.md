...
- `letter-flyer.html`: 8.5 × 11 in, 1 page. Destination Save as PDF, Margins at Default. Dark header band, tagline, three service photos, services with their one-line descriptions, contact and QR.
...
- `tri-fold.html`: 11 × 8.5 in, 2 pages. Destination Save as PDF, Margins at Default; the PDF is 11 × 8.5 in landscape. Print double-sided, flip on the short edge, fold the inner flap in first. Outside: Brandi Robinson's review with QR, contact panel, cover with van photo. Inside: why us, services, three steps and what to expect.
...
- missing (printed as dashed placeholders): `business.licence` in `letter-flyer.html` and `tri-fold.html`. Delete the `<span class="ph" …>` if there is no licence number to print, or add the number to `brand-facts.yaml` as `provided` and ask for a regenerate.
...
- placeholder inventory: `business.licence` × 2
...
| Dashed "licence number" text on the flyer or brochure | The `business.licence` placeholder | Delete that span in the HTML, or add the number to `brand-facts.yaml` and ask for a regenerate |
